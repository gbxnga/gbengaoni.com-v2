...
AWS has introduced the new Zero-ETL integration feature that facilitates the automatic shipping off
...
Make sure the database is publically available(we will establish connection with local PC shortly) and choose the default 
...
You will find this clicking on the cluster and scrolling down to the **Endpoints** section.
...
Click **Modify** on your the database and add the new security group. This will enable connection between you PC and the new instance.
...
This will enable ingestion of data directly into the database.
...
workgroup. This is beacuse, for now, the Zero-ETL integration is only available for testing purposes
only. 

![](/images/22-rds-31.png) 

On the Redshift serverless dashboard, you will see an alert plane asking you to try the
...
Redshift serverless is now available.
...
for Zero-ETL integration, specific values for the DB Cluster parameters are required - the Binary logging configuration.

A custom DB parameter group will be created and then it will be attached to the DB Cluster.
A Custome parameter group with the below settings is needed:
```
