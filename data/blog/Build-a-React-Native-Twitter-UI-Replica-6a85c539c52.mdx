...
summary: Let’s build a Twitter Lookalike with React Native
...
Great! Now that our environment is set up, lets get our hands dirty. We will install all dependencies needed to build our Twitter app. We will be making use of [React Navigation](https://reactnavigation.org/) — a routing and navigation helper for React Native apps built to replace the now deprecated `NavigatorExperimental` and the not so powerful `Navigator`, a JS-based solution launched with React Native.

We will also be using [Axios](https://github.com/axios/axios) — a promise based http client for javascript — to make api calls for data fetching, and also [React Native Vector Icons](https://www.npmjs.com/package/react-native-vector-icons).
...
*   Replace the yet to exist `Home` component that the **Home** route of our`HomeTabs` component points to with a page that displays ‘hello, this is your timeline’:
...
The timeline will be a `ListView`which we will give a padding at the top that equals the navigation bar — which will have an absolute positioning — in height.
...
```
We check the _clampedScrollValue_ whether or not it’s passed half of the navigation bar. We will also make sure we don’t hide it while we haven’t scrolled yet, to avoid showing a blank space where the navigation bar is supposed to be.
